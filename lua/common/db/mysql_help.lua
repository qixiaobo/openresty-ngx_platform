...
function _M.replace_help(tbname,param,PKname)
	local key=""
	local value = ""
	...
	local sql = string.format("replace into "..tbname.. "(%s) values(%s) %s ;",key,value, sqlwheresuffix)
	return sql;
end
...
--[[
	调用此方法生成MYSQL插入语句
	local db_help = require "db.lua_db_help"
    local strsql = db_help.insert_help(tbname,param)
...
function _M.insert_multi_help(tbname,param) 
	local key=""
	local value = ""
	local index1 = 0
	for _,v1 in pairs(param) do
		index1 = index1 + 1
		local value1 = ""
		for k,v  in pairs(v1) do
			...
			if type(v) == "table" then
				vTemp = cjson.encode(v)
			else
				vTemp = v
			end
			if(value1 == "") then
				if index1 == 1 then
					value1= "( " .. ngx.quote_sql_str(vTemp)
				else
					value1= ", ( " ..ngx.quote_sql_str(vTemp)
				end
			else
				value1 = value1..","..ngx.quote_sql_str(vTemp)
			end 
		end 
		value1 = value1.." )"
		value =  value .. value1
...
function _M.delete_help(tablename,param)
	...
	-- local sql = string.format("delete from"..tablename..t_usersqlwheresuffix) 
	local sql = string.format("delete from "..tablename.." %s;",sqlwheresuffix) 
	return sql
end 

--[[
结果判断调用 搜索查询,用于比对一条唯一数据或者查询不到的场景下
]]
function _M.select_one_result(_res,_err)
	-- body
...
--[[
结果判断调用 用于新增,修改,删除等场景,主要判断结果返回字段
]]
function _M.select_one_result(_res,_err)
	-- body
	if not _res then return nil,_err end


	if _res.affected_rows > 0  then
		return true,_res
	end 
	return false,_err
