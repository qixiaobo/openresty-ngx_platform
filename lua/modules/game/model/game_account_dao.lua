...
--[[
-- get_user_accounts 根据条件查询用户游戏账户信息 ,游戏账户分游戏, 区服,游戏为必带字段
-- example   
    local res = _M.get_user_accounts(_user_account, _start_index, _offsets, )
 
-- @param  _user_account 用户游戏账户搜索结构
...
--[[
-- get_user_user_account 根据条件查询用户游戏账户信息 ,游戏账户分游戏, 区服,游戏为必带字段, 用户code
-- example  
    
    local res = _M.get_user_accounts(_user_code, )
 
-- @param  _user_code 用户系统唯一编号 必需
-- @param  _game_code 游戏编号          必需
-- @param  _districts_code 游戏区服务编号
-- @param  _start_index 搜寻的数据的当前位置, 用于分页查询
-- @param  _offsets  

-- @return  true 或者 nil 代表错误
--]]
_M.get_game_account = function ( _user_code, _game_code, _districts_code ) 
    -- body 
    local mysql_cli = mysql_db:new();
    if not mysql_cli then 
        return nil,1041;
    end 
    local _dis_str = _districts_code and " and game_district_fk = '".._districts_code.."'" or ""
    local str = string.format("select t_game_account.* from t_game_account where t_game_account.user_code_fk = '%s' and game_type_fk='%s' %s;"
        ,_user_code , _game_code, _dis_str)  

    return mysql_db:exec_once(str)
end


--[[
-- add_user_account 开启用户游戏账户,需要用户指定游戏类型,区服(如果有需要的话)

-- example 
    local _user_account = {...}
    local res = _M.add_user_account(_user_account)

-- @param  _user_account 游戏账户基础结构
-- @return  返回成功或者失败标志
--]]
_M.add_user_account = function ( _user_account ) 
   ...
   local str = mysql_help.insert_help("t_game_account", _user_account) 
...
--[[
--  update_user_account 用户账户的数据修改根据各种活动,充值等产生, 原则上不可以直接调用该函数进行用户账户修改
...
----- 建议未来采用存储过程调用,实现该业务过程
    ...
    local sql3 = mysql_help.insert_help("t_user_account_tf",transaction)  
...
----- 建议未来采用存储过程调用,实现该业务过程
    ...
    local sql2= string.format("update t_game_account set balance=balance - %d where user_code_fk = '%s';",_from_code,_money)
    ...
    local sql3= string.format("update t_game_account set balance=balance + %d where user_code_fk = '%s';",_to_code ,_money)
    ...
    local transaction = {
        transaction_code = transaction_code,
        transaction_from = _from_code,
        transaction_from_type = "ZSGAMEACCOUNT",
        transaction_to = _to_code,
        transaction_to_type = "ZSGAMEACCOUNT", 
        balance = _money, 
    }  
    -- 添加流水记录
    -- local transaction_code = uuid_help:get64();  
    local sql4 = mysql_help.insert_help("t_game_account_tf",transaction)  
...
_M.delete_user_account = function ( _id_pk ) 
   ...
   local str = mysql_help.delete_help("t_game_account", {id_pk = _id_pk}) 
