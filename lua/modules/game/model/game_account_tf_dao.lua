...
_M.add_user_account_tf = function ( _user_account_tf ) 
    ...
    if _machine_bet_record.bet_money then
        sql2 = string.format("update t_game_account set balance = balance - %d  where user_code_fk = '%s' and balance - %d >= 0;",_machine_bet_record.bet_money, _machine_bet_record.user_code_fk,_machine_bet_record.bet_money)
    else
        sql2 = string.format("update t_game_account set integral = integral + %d  where user_code_fk = '%s';",_machine_bet_record.bet_rewards,_machine_bet_record.user_code_fk)
    end
    ...
    if tonumber(res["affected_rows"]) == 0 then
        mysql_cli:query("ROLLBACK;") 
         ngx.log(ngx.ERR,"bad result: account balance is less");
        return nil
    end

    -- 添加流水记录 
    _machine_bet_record.bet_flow_code = incr_help.get_time_union_id() 


    local sql3 = mysql_help.insert_help("t_machine_bet_records",_machine_bet_record)  
...
_M.recharge_user_account_tf = function ( _user_account_tf) 
    ...
    local sql1 = string.format("update t_game_account set balance = balance + %0.2f  where user_code_fk = '%s';",_user_account_tf.trade_to_amount, _user_account_tf.trade_to_id)
...
--[[
-- get_recently_user_account_tf 查询最近流水 充值流水
--  
-- example 
...
_M.get_recently_user_account_tf = function ( _user_code , _start_index, _offsets) 
    ...
    local sql1 = string.format([[
            select t_trade_tf.trade_no,t_trade_tf.order_no,t_trade_tf.trade_time,t_trade_tf.trade_status from t_trade_tf
