...
--[[
-- get_game_accounts 获得用户记录信息, 该信息减少
-- example   
    local res = _M.get_game_accounts(_user_account, _start_index, _offsets, )
...
_M.get_game_accounts = function ( _user_account, _start_index, _offsets) 
    -- body 
    local mysql_cli = mysql_db:new();
    if not mysql_cli then 
        return nil,1041;
    end 
    local str = mysql_help.select_help("select t_game_account.* from t_game_account ", _user_account,"and",_start_index, _offsets ) 
    return mysql_help:exec_once(str)
end
...
-- example  
    ...
    local res = _M.get_game_accounts(_user_code, )
...
-- @return  true 或者 nil 代表错误
--]]
_M.get_game_account = function ( _user_code ) 
    -- body 
    local mysql_cli = mysql_db:new();
    if not mysql_cli then 
        return nil,1041;
    end 
    local str = string.format("select t_game_account.* from t_game_account where t_game_account.user_code_fk = '%s';"
            ,_user_code)  
...
--[[
-- add_user_account 添加用户账户信息,游戏账户类型不同
--  
-- example 
    local _user_account = {...}
...
----- 建议未来采用存储过程调用,实现该业务过程
    ...
    local sql2= string.format("update t_game_account set %s where user_code_fk = '%s';",strimpl,_user_code)
    ...
    local sql3 = mysql_help.insert_help("t_game_account_tf",transaction)  
