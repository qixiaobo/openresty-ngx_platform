...
local mplayer = require "game.machine.machine_player"
local visitor = require "game.machine.visitor"
...
local is_visitor = false
--	没有用户编号 退出 
if not user_code or not user_token or user_code=="" or user_token=="" then
	-- 游客身份
    user_code = uuid_help:get64()
    user_token= "xxxxx"
    is_visitor = 1
end
...
ngx.log(ngx.ERR,"user_code :",user_code)
...
local res = player:player_init(user_code,user_token)
...
-- redis订阅,订阅返回
local channel_name = ZS_COIN_MACHINE_SUBSCRIPT_PRE_STR..user_code
...
player.redis_cli = red
player.subscript_map[channel_name] = channel_name
player.subscript_map[ZS_COIN_MACHINE_SNOTICE] = ZS_COIN_MACHINE_SNOTICE


--sub
local res, err = red:subscribe(channel_name, ZS_COIN_MACHINE_SNOTICE)
if not res then
...
red:incr(SYSTEM_ON_LINE_USERS..machine_on_line_users)
