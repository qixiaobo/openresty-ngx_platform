...
--  作者:Steven 
--  日期:2017-02-26
--  文件名:machine_player.lua
--  版权说明:南京正溯网络科技有限公司.版权所有©copy right.
--  机器玩家的功能操作对象,包括各类数据存储与管理
--  
--]]
...
_M.player_init = function( _self, _user_code, _user_token )
	...
	-- 1 判断用户code 和 用户 token信息是否有效 

	-- 2 判断用户是否在线,读取用户上一次在线的信息 如果用户在线,则将用户t下线
 	_self.online_uuion = uuid_help:get64() 
 	-- 生成channel 名称 进入房间之后,系统同样会给用户生成一个 room channel name
    ...
    _self.user_seat_machine_key = ZS_USER_MACHINE_SEAT_PRE.._user_code
	-- 进行唯一性登录判断 是否登录
	local res, last_info = online_union_help.set_online_redis(_self.user_online_key, _self.online_uuion, PLAYER_TIME_OUT)
	if not res then
		return nil
	end 
	...
	if res == 0 then
	    ...
	    local res, err = redis_cli:publish( _self.user_channel_name, cjson.encode(_process))
	    if not res then 
	    	ngx.log(ngx.ERR," player init ",_user_code," redis_cli:publish err: ",err) 
	    	return nil end   
	end

	-- 判断用户状态,是否禁用等,用户token是否有效
	local user_dao = require "user.model.user_dao" 
	local res  = user_dao.is_keep_alived_login(_user_code,_user_token) 
    if not res then
    	-- 系统错误
    	ngx.log(ngx.ERR,"token 无效!")
    	return nil,403
    end 
...
-- 读取系统基础信息
    local sql = string.format([[select t_user_ext_info.nick_name, t_game_account.* 
    		from t_game_account inner 
    		join t_user on t_user.user_code =  t_game_account.user_code_fk and t_user.user_code = '%s'
    		left join t_user_ext_info on t_user_ext_info.user_code_fk = t_game_account.user_code_fk ;]],_user_code)
    ...
    if #res ~= 1 then
	  	ngx.log(ngx.ERR,"bad result: no user_code ",_user_code, "."); 
        return nil,errcode;
    end 
    _self.nick_name = res[1].nick_name
	_self.balance = res[1].balance
	_self.nomove_balance = res[1].nomove_balance
	_self.integral = res[1].integral 
	_self.subscript_map = {}		-- 订阅列表
 	
 	if  not last_info or last_info == ""  then
 		_self.player_status = PLAYER_STATUS.FREE 	 -- process家状态
 	else 
 		ngx.log(ngx.ERR,"-----last_info -",last_info)
 		local res, last_info_t = pcall(cjson.decode,last_info)
 		if res then 
	 		_self.player_status = last_info_t.player_status 	-- 玩家状态
	 		_self.machine_code = last_info_t.machine_code
	 		_self.room_code = last_info_t.room_code	 
	 		_self.queue_machine_code = last_info_t.queue_machine_code
	 			
	 		_self.level = last_info_t.level
			_self.srv_id = last_info_t.srv_id 
 
	 		if _self.player_status == PLAYER_STATUS.ON_SEAT then
	 			local time_left = online_union_help.get_left_time(_self.user_seat_machine_key)
  				_self.seat_left_time = time_left 
	 		end


	 		local redis_cli = redis_help:new(); 
	 		if redis_cli then
				if not _self.room_counts then 
					redis_cli:incr(SYSTEM_ON_LINE_USERS.._self.room_code) 
					_self.room_counts = true 
				end 
	...
	local _data = {
		...
		online_uuion = _self.online_uuion, 
		seat_left_time = _self.seat_left_time,
		is_visitor = 0,
		level = _self.level,
		srv_id = _self.srv_id, 

	}
	ngx.log(ngx.ERR,"send msg",cjson.encode(_data))
	-- {process_type = PROCESS_TYPE.MACHINE_NOTICE_S2U,sub_type = xx, data = {},  user_code="xxx",machine_code="xxxx"} 
	local _process = {
	...
	-- online_union_help.update_online_redis(_self.user_online_key, cjson.encode(_data), 
	-- 	PLAYER_TIME_OUT)  
    return true

end

_M.save_user_info = function (_self,_time_out)
	local  data = {
		player_status 	= _self.player_status,
		user_code 		= _self.user_code,
		room_code 		= _self.room_code, 
...
_M.on_machine_sidelines_u2s = function (_self, _process )
	...
	if not _process.room_code or tostring( _process.room_code) == "" then 
		ngx.log(ngx.ERR,"player ",_self.user_code," on_machine_sidelines_u2s ", " ,_process.room_code: ", _process.room_code) 
	...
	_self.level = _process.level
	_self.srv_id = _process.srv_id
	...
	-- if _self.player_status ~= PLAYER_STATUS.QUEUE_UP then
...
_M.on_machine_outsidelines_u2s = function (_self, _process ) 
  	-- 断开请求 
	-- 1, 基础判断,参数和属性是否正确 
	if not _process.room_code 
		or tostring( _process.room_code) == "" 
		or _process.room_code ~= _self.room_code then 
		ngx.log(ngx.ERR,"player ",_self.user_code," on_machine_outsidelines_u2s ", " ,_process.room_code: ", _process.room_code) 
		_process.code = MACHINE_ERR_CODE.PARAM_ERROR
		_process.msg="room_code can not be  nil or nullstr"
		_self.ws:sendMsg(cjson.encode(_process)) 
	return nil end 
	...
	-- else
	-- 	_self.player_status = PLAYER_STATUS.FREE
	-- end
	_self.player_status = PLAYER_STATUS.FREE
	...
	local redis_cli = _self.redis_cli_1
	if redis_cli then
		if _self.room_counts then
			redis_cli:decr(SYSTEM_ON_LINE_USERS.._self.room_code)
			_self.room_counts = false
		end
...
_M.send2machine = function(_self, _process)
	...
	if res == 0 then
    	...
    	_process.process_type = PROCESS_TYPE.ASK_FOR_MATHINE_M2U 
    	_self.player_status = MACHINE_STATUS.SIDELINES
		_self.ws:sendMsg(cjson.encode(_process)) 
		return nil 
	end
...
_M.on_ask_for_machine_u2m_timer_func = function(_self,_process)
	-- body
 	if _process.process_type == PROCESS_TYPE.ASK_FOR_MATHINE_U2M and _self.player_status == PLAYER_STATUS.MACHINE_ASKING then
 		-- 超过五秒之后关闭通知用户请求失败
...
_M.on_ask_for_machine_m2u = function (_self, _process )
	...
	if _self.player_status ~= PLAYER_STATUS.MACHINE_ASKING then
		...
		local channel_name = ZS_COIN_MACHINE_SUBSCRIPT_PRE_STR.._process.machine_code
	    	...
	    	ngx.log(ngx.ERR,"-----------------1  MACHINE_ASKING error ")
		return nil
	end 
...
_M.on_ask_for_disconnect_m2u = function (_self, _process )
	...
	if _self.machine_code ~= _process.machine_code then 
		return true 
	end 
 	-- 客户端机器将进行二次判断 将数据返回服务器
...
_M.on_machine_error_u2m = function (_self, _process )
	...
	local channel_name = "machine_kefu_100001"
    local res, err = redis_cli:publish(channel_name, cjson.encode(_process))
    if not res then 
    	ngx.log(ngx.ERR,"player ",_self.user_code," on_ask_for_machine_u2m redis_cli:publish err: ",err) 
	...
	if res == 0 then
		ngx.log(ngx.ERR,"player publish false, the service is not online!")  
    	_process.msg = "player publish false, the service is not online!"  
    	_process.code = MACHINE_ERR_CODE.SERVICE_OFFLINE  
		_self.ws:sendMsg(cjson.encode(_process)) 
		return nil 
	end
    return true
...
_M.on_queue_up_u2s = function (_self, _process)
	-- body
	-- 1, 基础判断,参数和属性是否正确  
    if _self.player_status ~= PLAYER_STATUS.SIDELINES  then
    	return nil
    end
    local res = _self:send2machine(_process)
 	if res then
...
-- example 
	QUEUE_CANCEL_ON_U2S = 0x15,		-- 系统通知用户所在的机器准备好
 		-- {process_type=PROCESS_TYPE.QUEUE_CANCEL_ON_U2S, user_code="xxx",machine_code="xxxx", code=200}  
...
_M.on_queue_cancel_on_u2s = function (_self, _process)
	...
	_process.machine_code = _self.queue_machine_code
	_self.queue_machine_code = nil
	ngx.log(ngx.ERR,'----on_queue_cancel_on_u2s----',_process.machine_code)
	return _self:send2machine(_process)  
...
_M.on_client_offline_s2u = function (_self, _process )   
	ngx.log(ngx.ERR,"-------- 协议请求 ",_self.online_uuion)
...
_M.player_break_us2ms = function(_self)
    ...
    local res, err = redis_cli:publish(channel_name, cjson.encode(_process))
    if not res then 
    	ngx.log(ngx.ERR,"machine ",_self.machine_code," player_break_us2ms redis_cli:publish err: ",err) 
    	return nil end 
...
_M.on_wechat_msg_u2s = function (_self, _process ) 
	...
	_process.nick_name=_self.nick_name
...
--[[
-- on_player_cancel_reconnect 用户断开之后,再次进入系统,用户选择取消,系统清空用户状态与数据
-- example  
  	PLAYER_CANCEL_RECONNECT = 0x0d, -- 用户断开之后,再次进入系统,用户选择取消,系统清空用户状态与数据
	-- {process_type=PROCESS_TYPE.PLAYER_CANCEL_RECONNECT, }  
-- @param _self 当前用户对象
-- @param _process 协议结构表  
--]]
_M.on_player_cancel_reconnect = function (_self, _process ) 
  	-- 断开请求
  	-- 1, 基础判断,参数和属性是否正确  
 	_process.machine_code = _self.machine_code
 	ngx.log(ngx.ERR,"----------- ",_self.machine_code," ",_self.player_status)
  	...
  	elseif _self.player_status == PLAYER_STATUS.QUEUE_UP  then
  		local _dis_process = {
			process_type = PROCESS_TYPE.QUEUE_CANCEL_U2S,
			sub_type = 1, -- 
			user_code = _self.user_code,
			machine_code = _self.queue_machine_code,
		}
		...
		local channel_name = ZS_COIN_MACHINE_SUBSCRIPT_PRE_STR.._process.machine_code
	    	redis_cli:publish(channel_name, cjson.encode(_dis_process)) 
		return nil
	elseif _self.player_status == PLAYER_STATUS.SIDELINES then
		if _self.room_code then
	 		_self:unsubscribe_channel( ZS_ROOM_MSG_PRE_STR.._self.room_code ) 
		end 
  	end	--todo
...
--[[
-- on_system_notice_s2u  充值或其他消息到来的时候需要主动通知用户,当用户还在线的时候,系统自动通过websocket将消息推送给用户
	-- app的用户通过推送消息主动推送
-- example  
...
-- @param _process 协议结构表  
--]]
_M.on_system_notice_s2u = function (_self, _process ) 
  	 
	if _process.sub_type == PROCESS_SUB_TYPE.USER_STATUS then
		if  _process.data.balance_changed then
			_self.balance = _self.balance + _process.data.balance_changed
		 
		elseif  _process.data.integral_changed then
			_self.integral = _self.integral + _process.data.integral_changed
		end


	elseif _process.sub_type == PROCESS_SUB_TYPE.MAIL_COMING then
		-- 系统通知事件直接不处理,如果需要处理 可在本区域进行修改

	end	
	

	_process.data.balance = _self.balance
	_process.data.integral = _self.integral
	_self.ws:sendMsg(cjson.encode(_process))
...
_M.on_seat_u2m = function (_self, _process ) 
  	-- 用户状态不在游戏中,不可以进行留座操作
 	if _self.player_status ~= PLAYER_STATUS.ON_GAME or not _self.machine_code or
 	  _process.machine_code ~=  _self.machine_code  
 	  or not _process.minutes or tonumber(_process.minutes) < 0 or tonumber(_process.minutes) > 10  
 	  then  
	 	  _process.code = MACHINE_ERR_CODE.PARAM_ERROR
...
_M.on_seat_m2u = function (_self, _process ) 
  	...
  	if  _process.code ==  200 then
  		-- 开启定时器
  		_self.player_status = PLAYER_STATUS.ON_SEAT 

		online_union_help.set_online_redis(_self.user_seat_machine_key,
			_self.user_code, tonumber(_process.minutes*60))
		
		_self.balance = _self.balance - _process.balance
		_self:save_user_info(tonumber(_process.minutes*60))
  	end
...
_M.on_seat_cancel_u2m = function (_self, _process ) 
  	-- 用户状态不在游戏中,不可以进行留座操作
  	if _self.player_status ~= PLAYER_STATUS.ON_SEAT  and  _self.player_status ~= PLAYER_STATUS.ON_GAME  then
		_process.code = MACHINE_ERR_CODE.PARAM_ERROR
		_process.process_type = PROCESS_TYPE.SEAT_CANCEL_M2U
		_self.ws:sendMsg(cjson.encode(_process))
 		ngx.log(ngx.ERR,"player_status ",_self.player_status," ",PLAYER_STATUS.ON_SEAT)
 		return nil
  	end
  	_self.player_status = PLAYER_STATUS.ON_GAME 
  	_self:save_user_info()
...
_M.on_seat_cancel_m2u = function (_self, _process ) 
  	...
  	if  _process.code ==  200 then
		...
		local res = online_union_help.delete_online_redis(_self.user_seat_machine_key)
		if  not res  then   
			_self.player_status  = PLAYER_STATUS.ON_GAME
			_self.ws:sendMsg(cjson.encode(_process)) 
			return nil
...
_M.dispatch_process = function (_self,_process )
	...
	if _process.process_type ~= 0x01 then
       ngx.log(ngx.ERR,string.format("--player -- %s,uuid:%s function:%s , proecss:%s, player status %d",
        	_self.user_code,_self.online_uuion ,PROCESS_FUNC_MAP[_process.process_type],cjson.encode(_process), _self.player_status))
    end   
