...
function TFGameRoom:pref_clearing()
 -- 判断当前是否需要循环,即 只有一个用户时直接清算
    local on_game_players = 0
    for i=1,player_len do
        if self.player_list[i].player_state == PLAYER_STATE.ON_GAME or 
            self.player_list[i].player_state == PLAYER_STATE.CHECK or 
            self.player_list[i].player_state == PLAYER_STATE.CALL or 
            self.player_list[i].player_state == PLAYER_STATE.RAISED or  
            self.player_list[i].player_state == PLAYER_STATE.ALL_IN or 
            on_game_players = on_game_players + 1
            if on_game_players > 1 then
               return false
            end
        end
...
function TFGameRoom:notice_game_room(_channel_name,_protocol)
...
function TFGameRoom:waitting_player_cb()

   local curl_player = self.player_list[self.auth_person_index] 
   -- 超时进行,判断当前用户是否必须要raise 如果需要的话,并且不是自己,则直接fold
   if self.raise_player then
        if self.raise_player.player_index ~= curl_player.player_index then
            -- 模拟调用 fold 请求
...
function TFGameRoom:per_bet_loop()
    ...
    for i=1,player_len do
        ...
        elseif raise_player.player_index == curl_player.player_index and raise_player.is_default_raise then
            ...
            if raise_player.player_state == PLAYER_STATE.ALL_IN then
                ...
                -- 需要通知玩家,玩家拥有 check raise 权限
                auth_opt = {"check","raise"}
                break;
            end
        else
           ...
           if curl_player.player_state == PLAYER_STATE.ALL_IN 
                ...
                auth_opt = {"call","allin","fold","raise"}
                break; 
            end
        end  
    end
    ...
    local _protocol = {
        ...
        data = { 
            ...
            player_no = curl_player.player_no, 
            auth_opt = auth_opt,
        }
    }
    redis_publish(channel_for_player_room_no, cjson.encode(_protocol))
    -- 开启定时器
    game_timer = timer_help:new(self.waitting_player_cb,self)
...
function TFGameRoom:flop_bet_loop() 
    ...
    for i=1,player_len do
        ...
        if not raise_player then
             if curl_player.player_state == PLAYER_STATE.ALL_IN 
                    ...
                    auth_opt = {"allin","fold","raise","check"}
                    break; 
                end
        else 
            if raise_player.player_index == curl_player.player_index then 
                self:on_turn()
                return false 
            else 
               if curl_player.player_state == PLAYER_STATE.ALL_IN 
    ...
    local _protocol = {
        protocol_type = PROTOCOL_TH.ROOM_STATE_S2U,
        sub_type =  SUB_PROTOCOL.ON_FLOP_BET,
        data = { 
            room_no = self.room_no,
            innings_no = self.innings_no,
            game_state = self.game_state,
...
function TFGameRoom:turn_bet_loop() 
    -- 从第一个位置开始玩家如果是all状态/fold则自动往后一位
    local res = self:pref_clearing()
    if res then
        return 
...
function TFGameRoom:river_bet_loop() 
    -- 从第一个位置开始玩家如果是all状态/fold则自动往后一位
    local res = self:pref_clearing()
    if res then
        return 
...
function TFGameRoom:on_clear()
    ...
    for i=1,player_len then
        local player = player_list[i]
        if player.player_state == PLAYER_STATE.ALL_IN
            or player.player_state == PLAYER_STATE.ALL_IN
            or player.player_state == PLAYER_STATE.ALL_IN
        then
            table.insert(clear_players,player)
        end
    end
