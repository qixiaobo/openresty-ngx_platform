...
local THPlayer = { 
    ...
    -- 玩家游戏状态 主要表现为 离开 旁观 游戏 掉线等
    player_state = PLAYER_STATE.SIDELINES,
 
    -- 玩家状态
    game_state = PLAYER_GAME_STATE.ON_WAITING,

    -- 玩家所在游戏的状态
    tfgame_state = THGAME_STATE.ON_GAME_RESET,
...
-- 继承
THPlayer.__index = THPlayer
setmetatable(THPlayer,BasePlayer)
...
THPlayer._VERSION = '0.01'   

THPlayer._CLAZZ = "game.texas_holdem.player"  
...
function THPlayer:reset(_innings_no,_is_dealer,_is_small_blind,_is_big_blind,_is_utg)
...
function THPlayer:bet(_bet_money)
   if self.player_state == PLAYER_GAME_STATE.ALL_IN then
        return true
   end
   ...
   if _tm < 0 then 
        ...
        self.balance = 0 
        self.player_state = PLAYER_GAME_STATE.ALL_IN 
...
function THPlayer:on_heartbeat_c2s(_protocol) 
...
function THPlayer:on_heartbeat_s2c(_protocol)
...
function THPlayer:on_room_sidelines_u2s(_protocol)
...
function THPlayer:on_room_sidelines_s2u(_protocol)
...
function THPlayer:on_room_outsidelines_u2s(_protocol)
...
function THPlayer:on_room_outsidelines_s2u(_protocol)
...
function THPlayer:on_wechat_msg_u2s(_protocol)
...
function THPlayer:on_wechat_msg_s2u(_protocol)
...
function THPlayer:on_join_game_u2s(_protocol)
...
function THPlayer:on_join_game_s2u(_protocol)
    local _data = _protocol.data

    if _protocol.code == 200 then
        self.player_state = PLAYER_GAME_STATE.ON_WAITING
...
function THPlayer:on_left_game_s2u(_protocol)
    local _data = _protocol.data
...
function THPlayer:on_game_state_s2u(_protocol)
...
function THPlayer:on_player_cz_s2u(_protocol)
...
--]]
function THPlayer:dispatch_protocol( _protocol )
    -- body
...
-- @param _protocol 协议格式
--]]
function THPlayer:bet_mysql( _bet_money )
...
-- @param  _bet_money 用户押注金额 
--]]
function THPlayer:rewards_mysql( _rewards_money )
