...
_Poker.SUIT_TYPE = SUIT_TYPE


local SUIT_TYPE_ARRAY = {
    "SPADES" ,
    "HEARS"  ,
    "CLUBS"  ,
    "DIAMONS", 
}
_Poker.SUIT_TYPE_ARRAY = SUIT_TYPE_ARRAY


local SUIT_TYPE_MAP = {
    "♠️","♥️","♣️","♦️" 
}
_Poker.SUIT_TYPE_MAP = SUIT_TYPE_MAP
...
_Poker.CARD_ID_TYPE = CARD_ID_TYPE
--[[
    单张卡牌的id 与 value之间的映射
]]
local CARD_ID_KEY_MAP = {
            "ACE", "TWO", "THREE", "FOUR", "FIVE", "SIX", "SEVEN", "EIGHT",
            "NINE", "TEN", "JACK", "QUEEN", "KING", "BLACKJOKER", "REDJOKER"
}

_Poker.CARD_ID_KEY_MAP = CARD_ID_KEY_MAP

local CARD_ID_NAME_MAP = {
            "Ace", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight",
            "Nine", "Ten", "Jack", "Queen", "King", "BlackJoker", "RedJoker"
}
_Poker.CARD_ID_NAME_MAP = CARD_ID_NAME_MAP
...
_Poker.CARDS_COMPARE = CARDS_COMPARE
...
local PokerCard = require "game.poker_card"
...
-- 根据需要初始化的扑扑克返回扑克
    ...
    -- 需要大小王时,添加进入卡牌中
    if hasJoker then
        cards[1] =  PokerCard:new(nil, CARD_ID_TYPE.REDJOKER, CARD_POINT.REDJOKER);
        cards[2] =  PokerCard:new(nil, CARD_ID_TYPE.BLACKJOKER, CARD_POINT.BLACKJOKER);
        startIndex = 3; 
    end 
   
    for ks,vs in pairs(SUIT_TYPE) do
        for kid,vid in pairs(CARD_ID_TYPE) do
            if CARD_ID_TYPE.REDJOKER ~= vid and vid ~= CARD_ID_TYPE.BLACKJOKER then
                cards[startIndex] = PokerCard:new(vs,vid, CARD_POINT[kid],false); 
