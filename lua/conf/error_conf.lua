...
require "conf.three_platform_err"
require "conf.auth_err"
...
-- 正确执行
ZS_ERROR_CODE.RE_SUCCESS = 200;

-- 发生错误
ZS_ERROR_CODE.RE_FAILED = 400;
-- 系统故障 请稍后再试
ZS_ERROR_CODE.SYSTEM_ERR = 401;


-- 需要权限访问的错误
ZS_ERROR_CODE.RE_ACCEESS_ERR = 402;

-- 访问权限错误,禁止访问
ZS_ERROR_CODE.RE_FORBIDDEN_ERR = 403;

-- 参数错误
ZS_ERROR_CODE.PARAM_PATTERN_ERR = 404;

-- 参数为空的错误
ZS_ERROR_CODE.PARAM_NULL_ERR = 405; 


-- 数据库连接创建错误的错误
ZS_ERROR_CODE.MYSQL_ERR = 406; 
-- redis连接创建错误
ZS_ERROR_CODE.REDIS_NEW_ERR = 407; 
-- redis操作错误
ZS_ERROR_CODE.REDIS_OPT_ERR = 408; 

-- ws 错误定义
ZS_ERROR_CODE.DATA_ERR = 470
